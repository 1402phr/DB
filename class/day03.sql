...
FROM
    emp,
    (
    -- 부서번호와 부서의 평균급여만 기억하는 가상의 테이블을 만들자.
        SELECT
...
/*
    사원중 최소급여자가 속한 부서의 사원들 중 부서 평균급여보다 급여가 많은 사원들의 
    사원이름, 부서이름, 부서급여합계, 부서최대급여, 부서최소급여, 부서평균급여
    를 조회하세요.
*/
...
*/
SELECT
    TO_CHAR(hiredate, 'YYYY') 입사년도, AVG(sal) 평균급여, SUM(sal) 급여합계
FROM
    emp
GROUP BY
